...
fileName = f"delayPredictorLR{version}.joblib"
...
def trainLinearRegression():
    ...
    log(Path(files[0]).parent.name, True, logModel, version)

    for days in files[1:]:
        log(Path(days).parent.name, True, logModel, version)
        cnt += 1
    ...
    # Linear Regression
    log("Creating LinearRegression model...", False, logModel, version)
    ...
    log("---- Model parameters ----", False, logModel, version)

    log(f"Intercept: {model.intercept_:.6f}", False, logModel, version)
    log(f"Number of features: {len(model.coef_)}", False, logModel, version)
